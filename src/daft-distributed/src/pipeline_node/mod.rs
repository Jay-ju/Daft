    ...
    F: Fn(LocalPhysicalPlanRef) -> LocalPhysicalPlanRef + Send + Sync + 'static,
{
    ...
    context.insert("node_name".to_string(), node.context().node_name.to_string());
    if context.get("origin_node_name").is_none() {
        if let Some(prev) = prev_node_name {
            context.insert("origin_node_name".to_string(), prev);
        } else {
            context.insert("origin_node_name".to_string(), node.context().node_name.to_string());
        }
    }
