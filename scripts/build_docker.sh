...
BASE_IMAGE_NAME=${BASE_IMAGE_NAME:-"las-ai-qa-cn-beijing.cr.volces.com/las/las-base"}
...
FLASH_ATTN_URL=${FLASH_ATTN_URL:-"https://las-ai-qa-online.tos-cn-beijing.volces.com/qa/whl/flash_attn-2.8.3%2Bcu12torch2.7cxx11abiTRUE-${CP_VERSION}-${CP_VERSION}-linux_x86_64.whl"}
...
IMAGE_NAME=${IMAGE_NAME:-"las-ai-qa-cn-beijing.cr.volces.com/las/${DAFT_NAME}"}
CPU_TAG=${CPU_TAG:-"${DAFT_VERSION}-py${PYTHON_VERSION}-ubuntu${UBUNTU_VERSION}"}
GPU_TAG=${GPU_TAG:-"${DAFT_VERSION}-cu${CUDA_VERSION}-py${PYTHON_VERSION}-ubuntu${UBUNTU_VERSION}"}
