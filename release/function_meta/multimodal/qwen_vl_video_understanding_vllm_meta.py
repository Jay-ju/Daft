...
def get_meta() -> OpMetaModel:
    return OpMetaModel(
        Name="视频内容理解（Qwen VL 系列模型，支持大尺寸模型）",
        Clazz=QwenVLVideoUnderstandingVLLM,
        Category=Category.MULTI_MODAL,
        SubCategory=SubCategory.VIDEO_TO_TEXT,
        Tags=["视频理解", "多模态理解", "vllm"],
    )


def get_extra_meta() -> ExtraMetaModel:
    code = f"https://{OP_BUCKET}.tos-{OP_REGION}.volces.com/{OP_ENVIRONMENT}/operator_cards/{OP_VERSION}/qwen_vl_video_understanding_vllm/qwen_vl_video_understanding_vllm.py"
    code_description = "下面的代码展示了如何使用 daft 运行算子理解视频内容，并按照指令生成描述。"
    before = [
        DataItem(
            Type=ValueType.Video.name,
            Value=f"https://{OP_BUCKET}.tos-{OP_REGION}.volces.com/{OP_ENVIRONMENT}/operator_cards/{OP_VERSION}/qwen_vl_video_understanding_vllm/eating_56.mp4",
            Description="",
        ),
    ]
    after = [
