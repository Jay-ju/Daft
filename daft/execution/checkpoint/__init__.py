...
def create_checkpoint_filter_udf(num_buckets: int, actor_handles: list[ray.ActorHandle] | None):
...
def try_apply_filter_and_collect(
    ...
    try:
        ...
        if checkpoint_config is not None:
            ...
            if actor_handles:
                logger.info("Checkpoint filter enabled (Actor Mode)")
                checkpoint_filter = create_checkpoint_filter_udf(num_buckets, actor_handles)
                write_df = write_df._insert_filter_after_source(checkpoint_filter(col(key_column)))
            else:
