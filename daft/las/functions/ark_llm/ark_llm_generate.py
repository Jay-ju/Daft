...
class ArkLLMGenerate(Operator):
    ...
    **核心功能：**
    - 支持豆包系列模型和 DeepSeek 大模型，对输入的消息进行理解回复或推理
    - 提供通用的文本生成或视觉理解能力，输入字段格式需满足:[{'role': 'user', 'content': query}]类似的格式，详细格式请参考方舟大模型服务平台要求的messages格式（https://www.volcengine.com/docs/82379/1494384）
    ...
    **支持模型示例：**
    ...
    def transform(self, messages: pa.Array) -> pa.Array:
        ...
        Args:
            messages: 包含待处理消息的数组。类型为list[dict].
