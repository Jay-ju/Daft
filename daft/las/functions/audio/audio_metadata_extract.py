...
class AudioMetadataExtract(Operator):
    """音频元数据提取算子.
    ...
    def __init__(self, timeout: int | None = None, **kwargs: Any) -> None:
        ...
        try:
            ...
            self.logger = get_logger(f"AudioMetadataExtract-{id(self)}")
        ...
        self.logger = get_logger(f"AudioMetadataExtract-{id(self)}")
        self.logger.info("AudioMetadataExtract initialized with timeout=%s", self.timeout)
