...
class AudioConvert(Operator):
    """**通用音频格式转换处理器，支持转换为任意音频格式**

    **核心功能：**
    - 支持转换为多种音频格式（MP3、WAV、FLAC、AAC、OGG等）
    - 音频采样率自定义
    - 智能音轨选择（全部/自动/首个）
    - 通过extra_params支持格式特定参数
    - 支持本地文件、HTTP/HTTPS URL和TOS/S3存储

    **格式支持：**
    - 输入：WAV、FLAC、MP3、AAC、M4A、OGG、WMA、APE等主流音频格式
    - 输出：由output_format参数指定（mp3, wav, flac, aac, ogg, m4a等）
    - 采样率：8kHz-192kHz
    """  # noqa: D415
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            output_format: 输出音频格式，如"mp3"、"wav"、"flac"、"aac"、"ogg"等
                该参数决定输出文件的扩展名和默认编码器
            sample_rate: 音频采样率，如22050、44100、48000、96000、192000，为None时保持原始采样率
                默认值：None
            audio_map: 音轨选择策略：
                - "all"：选择所有音轨（默认）
                ...
                - "first"：仅选择第一个音轨
                默认值："all"
            extra_params: 额外的ffmpeg参数列表，用于指定格式特定的编码器和参数
                例如：
                - MP3: ["-c:a", "libmp3lame", "-b:a", "192k", "-q:a", "2"]
                - WAV: ["-c:a", "pcm_s24le"]
                - FLAC: ["-c:a", "flac", "-compression_level", "8"]
                - AAC: ["-c:a", "aac", "-b:a", "128k"]
                默认值：None（使用ffmpeg自动选择的默认编码器）
        ...
        self.logger.info(
            "AudioConvert initialized with output_format=%s, sample_rate=%s, " "audio_map=%s, extra_params=%s",
            output_format,
            sample_rate,
            audio_map,
            self.extra_params,
    ...
    def process(self, input_path: str, output_path: str) -> str | None:
        ...
        tmp_out = None
        try:
            tmp_out = tempfile.NamedTemporaryFile(suffix=f".{self.output_format}", delete=False)
            tmp_out.close()
            ...
            # 用户自定义参数（包括编码器和格式特定参数）
            cmd += self.extra_params

            # 输出设置
            cmd += ["-loglevel", "error", tmp_out.name]

            self.logger.info("[AudioConvert] Running command: %s", " ".join(cmd))
            ...
            self.log_progress()
            self.logger.error("[AudioConvert] Failed: %s", e)
            return None
        finally:
